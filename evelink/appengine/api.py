...
class AppEngineAPI(api.API):
    ...
    def get_async(self, path, params=None):
        ...
        request = api.APIRequest.from_api(self, path, params)
        key = self._cache_key(request)
        response = yield self.cache.get_async(key)
        cached = response is not None

        if not cached:
            response = yield self.send_request_async(request)

        try:
            result = self.process_response(response)
            current_time, expires_time = result[1:]
        except api.APIError as e:
            current_time, expires_time = e.timestamp, e.expires
            raise e
        finally:
            if not cached:
    ...
    def send_request(self, request):
        ...
        params:
            ...
            otherwise a GET.
        """
        return self.send_request_async(request).get_result()
    ...
    def send_request_async(self, request):
        ...
        result = yield ctx.urlfetch(
            ...
            payload=request.encoded_params,
            method=urlfetch.POST if request.params else urlfetch.GET,
